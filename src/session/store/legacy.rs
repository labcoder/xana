use crate::session::record::{RecordEnvelope, SESSION_RECORD_VERSION, SessionRecord};
use crate::{
    bounded_file,
...
impl LegacyStore {
    ...
    pub(super) fn inspect_bytes(bytes: &[u8]) -> Result<LoadedSession, SessionError> {
...
impl fmt::Display for SessionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Protected(reason) => write!(formatter, "protected session storage: {reason}"),
