...
|---|---|---|
| Runtime to Desktop | A 256-item typed channel carries updates. A data-free coalescing signal wakes GPUI only after an update is queued. | `DesktopWakeSignal`; wake-without-polling regression fixture |
...
| Raster previews | Only the newest eight eligible previews totaling at most 20 MiB are admitted. Snapshot replacement evicts entries outside that window. | count, byte, overflow, duplicate, and eviction fixtures |
...
renderer and platform backend own those effects. The source-byte admission
window is an additional hard application bound, not a claim that compressed
bytes equal decoded GPU bytes.
