    ...
    pub fn main(out_dir: &str, out_path: &Path) {
        ...
        if cfg!(feature = "bundled-sqlcipher") {
            ...
            if cfg!(feature = "bundled-sqlcipher-vendored-openssl") {
                ...
                // The upstream Windows static archive refers to this PDB by basename.
                // Cargo moves the archive into an rlib in `deps`; preserve its matching
                // debug information there as well (do not suppress LNK4099).
                if target.contains("-windows-msvc") {
                    let root = PathBuf::from(env::var("DEP_OPENSSL_ROOT").unwrap());
                    let pdb = root.join("lib/ossl_static.pdb");
                    if pdb.is_file() {
                        let profile_dir = Path::new(out_dir).ancestors().nth(3).unwrap();
                        let deps = profile_dir.join("deps");
                        std::fs::create_dir_all(&deps).unwrap();
                        std::fs::copy(pdb, deps.join("ossl_static.pdb")).unwrap();
                    }
                }
